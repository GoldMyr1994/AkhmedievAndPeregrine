...
tshift1=-10;
FIN1 = 1*amp*exp(-(t+tshift1).^2/(2*tau0^2));

tshift2=0;
FIN2 = 1*amp*exp(-(t+tshift2).^2/(2*tau0^2));

tshift3=10;
FIN3 = 1*amp*exp(-(t+tshift3).^2/(2*tau0^2));
...
for loop_step=2:1:nz
    ...
    if (z(loop_step)<(z1/2)); beta2=beta21; else; beta2=beta22; end
...
figure
plot(t,abs(F(:,1)),t,abs(F(:,end)),t,abs(F(:,length(z)/2)))
xlabel('t')
ylabel('|F|')
legend('IN','OUT')
set(findall(gcf,'type','text'),'FontSize',30)
